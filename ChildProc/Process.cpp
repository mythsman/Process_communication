...
INT_PTR CALLBACK DlgProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDC_BUTTON1:
			HWND nextHwnd;
			nextHwnd = FindWindow(L"Father Process", L"Father Process");
			wchar_t data[1000];
			SendMessage(GetDlgItem(dialog, IDC_EDIT1), WM_GETTEXT, 1000, (LPARAM)data);
			SendMessage(nextHwnd, LB_ADDSTRING, 1, (LPARAM)data);
			break;
		case IDC_BUTTON2:
			nextHwnd = FindWindow(L"Father Process", L"Father Process");
	...
	)
{
	...
	case LB_ADDSTRING:{
		SendMessage(GetDlgItem(dialog, IDC_LIST1), uMsg, NULL, lParam);
		return 0;
	}
	}
...
{
	WCHAR* cls_Name = L"Child Process";
